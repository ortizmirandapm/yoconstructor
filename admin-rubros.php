...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear') {
        ...
        if (!$nombre) {
            ...
            mysqli_query($conexion, "INSERT INTO rubros (nombre, descripcion, estado) VALUES ('$nombre','$desc',$estado)");
            $ok_msg = 'Rubro creado correctamente.';
        }
    }
    ...
    if ($accion === 'editar') {
        ...
        if (!$nombre) {
            ...
            mysqli_query($conexion, "UPDATE rubros SET nombre='$nombre', descripcion='$desc', estado=$estado WHERE id_rubro=$id");
            $ok_msg = 'Rubro actualizado correctamente.';
        }
    }
    ...
    if ($accion === 'toggle') {
        ...
        mysqli_query($conexion, "UPDATE rubros SET estado=$nuevo WHERE id_rubro=$id");
        $ok_msg = $nuevo ? 'Rubro activado.' : 'Rubro desactivado.';
    }

    if ($accion === 'eliminar') {
        $id  = intval($_POST['id']);
        $uso = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT COUNT(*) c FROM empresa WHERE id_rubro=$id"))['c'] ?? 0;
        if ($uso > 0) {
            $err_msg = "No se puede eliminar: $uso empresa(s) usan este rubro.";
        } else {
            mysqli_query($conexion, "DELETE FROM rubros WHERE id_rubro=$id");
            $ok_msg = 'Rubro eliminado correctamente.';
        }
    }
}
...
$where = ["1=1"];
if ($buscar)           $where[] = "nombre LIKE '%" . mysqli_real_escape_string($conexion, $buscar) . "%'";
if ($estado === 'activo')   $where[] = "estado=1";
if ($estado === 'inactivo') $where[] = "estado=0";
$wsql = implode(' AND ', $where);

$total_res   = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT COUNT(*) c FROM rubros WHERE $wsql"))['c'] ?? 0;
$total_pages = ceil($total_res / $per_page);
$res         = mysqli_query(
    $conexion,
    "SELECT r.*, (SELECT COUNT(*) FROM empresa WHERE id_rubro=r.id_rubro) AS uso
     FROM rubros r WHERE $wsql ORDER BY nombre ASC LIMIT $per_page OFFSET $offset"
);

$cnt_activos   = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT COUNT(*) c FROM rubros WHERE estado=1"))['c'] ?? 0;
$cnt_inactivos = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT COUNT(*) c FROM rubros WHERE estado=0"))['c'] ?? 0;
...
<div class="min-h-screen bg-gray-50 p-6 md:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Rubros</h1>
            <p class="text-gray-500 text-sm mt-0.5"><?= $total_res ?> rubro(s) encontrado(s)</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-200 bg-gray-50">
                    ...
                    <th class="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Empresas</th>
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                <?php if (!$res || mysqli_num_rows($res) === 0): ?>
                    <tr>
                        <td colspan="5" class="px-5 py-12 text-center">
                            <p class="text-gray-400 text-sm">No hay rubros<?= $estado === 'inactivo' ? ' inactivos' : ($estado === 'activo' ? ' activos' : '') ?></p>
                            <?php if ($buscar): ?>
                ...
                <?php else: while ($r = mysqli_fetch_assoc($res)): ?>
                    <tr class="hover:bg-gray-50 transition <?= $r['estado'] ? '' : 'opacity-60 bg-gray-50/50' ?>">
                        <td class="px-5 py-4">
                            <p class="font-semibold text-gray-800"><?= htmlspecialchars($r['nombre']) ?></p>
                        </td>
                        <td class="px-5 py-4 hidden md:table-cell">
                            <p class="text-xs text-gray-500 max-w-xs line-clamp-1">
                                <?= htmlspecialchars($r['descripcion'] ?: 'Sin descripción disponible') ?>
                            </p>
                        </td>
                        ...
                        <td class="px-4 py-4 text-center">
                            <div class="relative inline-block text-left">
                                <button type="button"
                                    onclick="toggleDropdown(event, 'drop-rubro-<?= $r['id_rubro'] ?>')"
                                ...
                                <div id="drop-rubro-<?= $r['id_rubro'] ?>"
                                    ...
                                    <div class="py-1">
                                        <button type="button"
                                            onclick="abrirModalToggle(<?= $r['id_rubro'] ?>,'<?= htmlspecialchars(addslashes($r['nombre'])) ?>',<?= $r['estado'] ?>)"
                                            ...
                                            <?= $r['estado'] ? 'Desactivar rubro' : 'Activar rubro' ?>
                                        </button>
                                        <button type="button"
                                            onclick="abrirModalForm('editar',<?= $r['id_rubro'] ?>,'<?= htmlspecialchars(addslashes($r['nombre'])) ?>','<?= htmlspecialchars(addslashes($r['descripcion'])) ?>',<?= $r['estado'] ?>)"
                                    ...
                                    <div class="py-1">
                                        <button type="button"
                                            onclick="abrirModalEliminar(<?= $r['id_rubro'] ?>,'<?= htmlspecialchars(addslashes($r['nombre'])) ?>',<?= $r['uso'] ?>)"
...
<div id="modalEliminar" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div class="px-6 py-5 border-b border-gray-200 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-gray-900">Eliminar rubro</h3>
...
<script>
    ...
    function abrirModalForm(modo, id = '', nombre = '', desc = '', estado = 1) {
        document.getElementById('mf-titulo').textContent = modo === 'crear' ? 'Nuevo rubro' : 'Editar rubro';
    ...
    function abrirModalToggle(id, nombre, estadoActual) {
        ...
        document.getElementById('mt-titulo').textContent = activo ? 'Desactivar rubro' : 'Activar rubro';
        document.getElementById('mt-nombre').textContent = nombre;
        document.getElementById('mt-mensaje').innerHTML = activo ?
            `¿Estás seguro de que querés <strong>desactivar</strong> este rubro?` :
            `¿Estás seguro de que querés <strong>activar</strong> este rubro?`;
    ...
    function abrirModalEliminar(id, nombre, uso) {
        ...
        document.getElementById('me-mensaje').innerHTML = uso > 0 ?
            `<span class="text-red-600 font-semibold">No se puede eliminar:</span> este rubro está siendo usado por <strong>${uso} empresa(s)</strong>.` :
